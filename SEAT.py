...
class target():
    def __init__(self,Insured_sex=[0.4,0.6],Region=[0.1,0.9],
                 Insured_age=[0.16,0.21,0.24,0.22,0.17],Car_use=[0.175,0.5166,0.005,0.3034]):
...
#SEAT.target([0.4,0.6],[0.1,0.9],[0.16,0.21,0.24,0.22,0.17],[0.175,0.5166,0.005,0.3034],100,0.1).conditional_D()
    ...
    def sample(self,N,random_state):
        ...
        while i < N:
            ...
            for j in range(len(self.conditional_D())):
                ...
                np.random.seed(random_state+i+1+1000000*(j+1))
    ...
    def fit(self,N,sigma,random_state):
        try:
            newdata = pd.read_pickle("draft.pkl")
            if N!=len(newdata):
                newdata=self.sample(N,random_state)
